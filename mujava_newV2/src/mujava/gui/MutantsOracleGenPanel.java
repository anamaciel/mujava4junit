...
public class MutantsOracleGenPanel extends JPanel 
{
	...
	JTable signatureOpTable = new JTable();
	JButton signatureNoneB = new JButton("None");
	JButton signatureAllB = new JButton("All");

	JTable annotationOpTable = new JTable();
	JButton annotationNoneB = new JButton("None");
	JButton annotationAllB = new JButton("All");

	public MutantsOracleGenPanel(GenMutantsMain parent_frame)
	{
		try 
		{
	...
	void jbInit()
	{
		...
		usgeContentP.add(new JLabel(" [2] Select oracle mutation operators to apply"));
		...
		FileOracleTableModel fTableModel = new FileOracleTableModel(MutationSystem.getNewTargetOracleFiles());
		...
		JTextField titleOP= new JTextField(" Oracle Mutation Operator");
		...
		AOOTableModel amTableModel = new AOOTableModel();
		signatureOpTable = new JTable(amTableModel);
		initColumnSizes(signatureOpTable,amTableModel);
		traditional_operator_scrollP.getViewport().add(signatureOpTable, null);
		...
		traditionalBPanel.add(signatureNoneB);
		signatureNoneB.addMouseListener(new java.awt.event.MouseAdapter()
		{
			public void mouseClicked(MouseEvent e)
			{
				signatureNoneB_mouseClicked(e);
			}
		});
		traditionalBPanel.add(signatureAllB);
		signatureAllB.addMouseListener(new java.awt.event.MouseAdapter()
		{
			public void mouseClicked(MouseEvent e)
			{
				signatureAllB_mouseClicked(e);
			}
		});
...
/**
 * <p>Description: Template containing annotation-level mutation operators
 *                 and signature-level mutation operators</p>
 */
